...
import { ICreateTopicRequest, ITopic, IUpdateTopicRequest } from '../topic.type';
import { toast } from '@/hooks/use-toast';
import topicService from '@/services/topic/topic.service';
import { TopicCreatedModal } from './TopicCreatedModal';
import { TopicUpdatedModal } from './TopicUpdatedModal';
import { TopicDeletedModal } from './TopicDeletedModal';
...
const TopicLibrary: React.FC<Props> = ({ classId, className }) => {
    ...
    } = useFetch<ITopic[]>(classId ? `/classes/${classId}/topics` : '/topics');
    ...
    const applyCreateTopic = (topic: ICreateTopicRequest) => {
    ...
    const applyUpdateTopic = (topic: IUpdateTopicRequest) => {
        if (topics === null || topics === undefined) return;
    ...
    return (
        <div className="w-full max-w-[85%] mx-auto mb-12 p-6 rounded-lg bg-gray-100 shadow-md dark:bg-gray-800">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <h2 className="text-2xl font-semibold">{classId ? className : t('title')}</h2>
                {isTeacher ? (
