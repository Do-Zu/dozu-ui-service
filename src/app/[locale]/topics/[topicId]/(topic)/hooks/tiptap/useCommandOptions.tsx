...
export default function useCommandOptions({ imageUploadButtonRef }: Props) {
    const options: Omit<CommandOptionProps, 'id'>[] = [
        {
            title: 'Text',
    ...
    ] as Omit<CommandOptionProps, 'index'>[];

    const result: CommandOptionProps[] = Array.from({ length: options.length }, (_, i) => i).map((index) => {
