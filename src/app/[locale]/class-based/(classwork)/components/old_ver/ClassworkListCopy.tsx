...
function ClassworkListCopy({ myClass, topics: topicsData, assignments }: Props) {
    const router = useRouter();
    ...
    // fetch classwork
    const { data, error, loading: classWorkLoading } = useFetch<any>(`/classes/${myClass.classId}/classwork`);
                ...
                <div className="mt-10">
                    ...
                    <div className="flex flex-col divide-y divide-border">
                        <>
                            ...
                            {/* tempdisplay */}
                            {data?.learningMaterials?.map((material: any) => (
                                <LearningMaterialItem key={material.learningMaterialId} material={material} />
                            ))}
                        </>
                    </div>
                </div>
                    ...
                    {assignmentsByTopic ? (
                        <div className="space-y-10 mt-10">
                            {topics.map((topic) => {
                                ...
                                if (topic.topicId === -1) {
                                    learningMaterials = data?.learningMaterials?.filter(
                                        (learningMaterial: any) => learningMaterial.topicId == null,
                                    );
                                } else {
                                    learningMaterials = data?.learningMaterials?.filter(
                                        (learningMaterial: any) => learningMaterial.topicId == topic.topicId,
                                    );
                                }
                                ...
                                return (
                                    <div key={topic.topicId}>
                                        ...
                                        <div className="flex flex-col divide-y divide-border">
                                            ...
                                            {learningMaterials?.map((material: any) => (
                                                <LearningMaterialItem
                                                    key={material.learningMaterialId}
                                                    material={material}
                                                />
                                            ))}
                                        </div>
...
export default React.memo(ClassworkListCopy);
