...
export default function ClassQuizItem({ role, quiz, classIdForRoute }: Props) {
  ...
  const statusBadge =
    quiz.status === 'draft' ? <Badge variant="secondary">Draft</Badge> :
    quiz.status === 'scheduled' ? <Badge className="bg-amber-500/20 text-amber-600 dark:text-amber-400">Scheduled</Badge> :
    quiz.status === 'published' ? <Badge className="bg-emerald-500/20 text-emerald-600 dark:text-emerald-400">Published</Badge> :
  ...
  return (
    <div className="flex items-center justify-between py-5 px-3 hover:bg-muted/50 rounded-md">
      <div className="flex items-center gap-4">
        <div className={cn('flex items-center justify-center h-9 w-9 rounded-full border-2',
          quiz.status === 'draft' ? 'border-muted-foreground/50 bg-muted/30' : 'border-emerald-400 bg-emerald-50 dark:bg-emerald-950/30')}>
          <ClipboardCheck className={cn(quiz.status === 'draft' ? 'text-muted-foreground/80' : 'text-emerald-500')} size={18}/>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {role === USER_ROLES.USER ? (
          <Button size="sm" onClick={() => router.push(ROUTES.STUDENT.CLASS_BASED_ID_CLASS_QUIZ_START(classIdForRoute!, quiz.classQuizId))}>
            ...
            <DropdownMenuContent align="end">
              ...
              {quiz.status === 'published' && quiz.acceptingSubmissions ? (
                <DropdownMenuItem onSelect={() => classQuizTeacherService.pause(quiz.classQuizId)}>
                  <Pause className="mr-2 h-4 w-4"/><span>Pause submissions</span>
                </DropdownMenuItem>
              ) : quiz.status === 'published' ? (
                <DropdownMenuItem onSelect={() => classQuizTeacherService.resume(quiz.classQuizId)}>
                  <PlayCircle className="mr-2 h-4 w-4"/><span>Resume submissions</span>
                </DropdownMenuItem>
              ) : null}
              {quiz.status !== 'closed' ? (
                <DropdownMenuItem onSelect={() => classQuizTeacherService.close(quiz.classQuizId)}>
                  <ShieldX className="mr-2 h-4 w-4"/><span>Close quiz</span>
                </DropdownMenuItem>
              ) : null}
            </DropdownMenuContent>
          </DropdownMenu>
