...
export default function HistoryPage() {
            ...
            <div className="container mx-auto py-10 max-w-3xl">
                ...
                {attemptList.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        {attemptList.map((at) => (
                            <Card key={at.attemptId} className="p-4 hover:bg-muted/40 transition rounded-xl">
                                <div className="flex items-center justify-between">
                                    {/* LEFT */}
                                    <div>
                                        <p className="font-semibold text-lg">Attempt #{at.attemptId}</p>
    ...
    const attemptMeta = attemptList.find((a) => a.attemptId === attemptId);
            ...
            <Card className="shadow-md rounded-xl">
                <CardHeader>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Submitted at:{' '}
                        {attemptMeta?.submittedAt ? new Date(attemptMeta.submittedAt).toLocaleString() : '—'}
                    </p>
                </CardHeader>
                ...
                <CardContent>
                    <div className="flex items-center gap-10">
                        ...
                        <div className="relative h-24 w-24 flex items-center justify-center">
                            <svg className="absolute h-24 w-24 transform -rotate-90">
                                <circle cx="48" cy="48" r="40" className="stroke-muted stroke-[8]" fill="transparent" />
                                <circle
                                    cx="48"
                                    cy="48"
                                    r="40"
                                    ...
                                    strokeDashoffset={2 * Math.PI * 40 - (2 * Math.PI * 40 * score) / 100}
                                    strokeLinecap="round"
                                />
                            </svg>
                    ...
                    <Button
                        variant="outline"
                        className="mt-6"
                        onClick={() => router.push(`/class-based/${classId}/class-quiz/${quizId}/history`)}
                    >
                        Back to attempts
                    </Button>
                </CardContent>
