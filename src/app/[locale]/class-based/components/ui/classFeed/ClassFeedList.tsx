...
export default function ClassFeedList(props: Props) {
    const { feedList, editable } = props;
        ...
        <div className="w-full h-full">
            <div className="space-y-4">
                {feedList.map((item, index) => {
                    const { classFeedId, title, content, createdAt, updatedAt, sender, link } = item;
                    return (
                        <Card key={index}>
                            <CardHeader>
                                {sender ? (
                            ...
                            {!sender ? (
                                <CardFooter>
                                    <div className="flex flex-col gap-2">
                                        ...
                                        {updatedAt ? (
                                            <span className="text-xs text-muted-foreground">
                                                This post was last edited at '
                                                {formatDate(updatedAt, DATETIME_DMY_12H_FORMAT)}'
                                            </span>
                                        ) : null}
                                    </div>
                                </CardFooter>
