...
import { getPostLoginRedirect, getPostRegistrationRedirect, RedirectService } from '@/utils/auth/redirectService';
...
export function useAuthNavigation() {
     ...
     * @param role - The role to check for ('admin', 'teacher', 'user')
     * @returns Boolean indicating if the user has the specified role
     */
    const hasRole = useCallback((user: User | null, role: string): boolean => {
        if (!user?.roles || !Array.isArray(user.roles)) {
            return false;
        }

        const normalizedRole = role.toLowerCase().trim();
        return user.roles.some((userRole) => userRole?.toLowerCase().trim() === normalizedRole);
    }, []);
    ...
    /**
     * Checks if a user has teacher role (case-insensitive)
     * @param user - The user object to check
     * @returns Boolean indicating if the user is a teacher
     */
    ...
    const getRedirectForRoute = useCallback(
        (path: string) => {
            try {
                ...
                const { destination } = RedirectService.shouldRedirect(pathWithoutLocale, userType, isAuthenticated);
