...
const Calendar = ({
    ...
    const {
        ...
        error,
        data: response,
    } = usePost<ScheduleGenerateRequest, ScheduleData>(generateSchedule, 'POST');
    ...
    const handleResponseGetSchedule = useCallback(() => {
        if (!response) {
            toast({
                description: 'Schedule data not found for the selected week.',
            });
            return;
        }
        ...
        if (!schedules || Object.keys(schedules).length === 0) {
            toast({
                description: 'No schedules found for the selected week.',
            });
            setEvents([]);
            return;
        }
        ...
        if (schedules) {
            const dataEvents = Object.values(schedules).flatMap((day) => {
                return day?.map(
                    (event, index) =>
                        ({
