...
export const DialogImportFileGenerative: React.FC<FileUploadDialogProps> = ({
  ...
  const cancelUpload = useCallback(
    (fileId: string) => {
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === fileId ? { ...f, status: 'error' as const, error: 'Upload cancelled' } : f,
        ),
      );
    },
    [files],
  ...
  const retryUpload = useCallback(
    async (fileId: string) => {
      ...
      try {
        const uploadId = await uploadService.uploadFile(fileItem.file, (progress) => {
          setUploadProgress((prev) => {
            const newMap = new Map(prev);
