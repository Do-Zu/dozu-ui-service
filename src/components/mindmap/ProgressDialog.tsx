...
const ProgressDialog = ({ nodeId, classId }: { nodeId: string; classId: number }) => {
    ...
    useEffect(() => {
        const fetchFlashcardsByNode = async () => {
            try {
                const result = await Axios.get(`/mindmap/${classId}/${nodeId}/class-progress`);
        ...
        <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>Flashcard mastery by user in this node.</DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4">
                    {summaryData.length === 0 ? (
                        ...
                        summaryData.map((summary) => {
                            ...
                            return (
                                <div key={summary.userId} className="border rounded-lg p-3 shadow-sm bg-white">
                                    <p className="font-medium">{summary.userName ?? `User ${summary.userId}`}</p>
